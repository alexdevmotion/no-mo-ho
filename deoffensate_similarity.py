...
def deoffensate_word_similarity_approach(text, token, nlp, token_parser):
    most_similar_words = most_similar_spacy(token.text, nlp, num_items=100)
    ...
    for replacement_token in tokens_filtered_by_pos:
        ...
        if replacement_token.text not in new_offesive_token_texts:
            ...
            print(replacement_token.text, 'replaces', token.text, '-', new_text)
        else:
            print(replacement_token.text, 'DOES NOT REPLACE', token.text)
